...
execute as @s[tag=mm_shoot] unless score survivor_left mm_data matches 1 run scoreboard players operation @s mm_sc_tick = shoot_cooldown_normal mm_settings

execute as @s[tag=mm_shoot] unless score survivor_left mm_data matches 1 store result score this_arrow_count mm_main run function mm:utils/get_count_in_inventory {id:"arrow"}
execute as @s[tag=mm_shoot] unless score survivor_left mm_data matches 1 run clear @s arrow
execute as @s[tag=mm_shoot] unless score survivor_left mm_data matches 1 run loot replace entity @s hotbar.1 loot mm:disabled_arrow
...
tellraw @s {"translate":"mm.text.collected_shot","color":"yellow","with":[{"text":"6"},{"text":"24"}]}
title @s subtitle {"translate":"mm.title.collected_shot.subtitle","color":"green","with":[{"text":"24"}]}
title @s title ""

function mm:utils/give_or_replace/main {id:"arrow",count:24,slot:1}

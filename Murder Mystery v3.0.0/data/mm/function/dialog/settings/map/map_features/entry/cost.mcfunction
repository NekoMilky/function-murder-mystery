$data modify storage mm:main dialog.dialog set value {\
  ...
  "body": [\
    {\
      ...
      "type": "plain_message",\
      "contents": {\
        "translate": "mm.text_settings.map.map_features.cost.value",\
        "color": "green",\
        "with": [\
          {\
            "text": "$(current_cost)",\
  ...
  "inputs": [\
    {\
      ...
      "type": "text",\
      "label": {\
        "translate": "mm.text_settings.map.map_features.cost.input"\
      }\
    }\
  ],\
  "yes": {\
...
$data modify storage mm:main temporary.string_1 set value "/function mm:settings/map/map_features/cost/modify {predicate:$(predicate),name:$(name),"
data modify storage mm:main temporary.string_2 set value "cost:$(missing)$(cost)}"

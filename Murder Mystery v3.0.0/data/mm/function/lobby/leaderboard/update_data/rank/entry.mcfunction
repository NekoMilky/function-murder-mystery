...
$execute unless score $(name) mm_temporary = max_value mm_main run return 0

#设置内容
$function mm:lobby/leaderboard/update_data/handler/$(handler) with storage mm:main temporary
data modify storage mm:main temporary.name_color set value "white"
...
$scoreboard players reset $(name) mm_temporary

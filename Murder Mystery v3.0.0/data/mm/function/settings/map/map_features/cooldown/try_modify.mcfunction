...
$execute at @s store result storage mm:main temporary.current_cooldown int 1 run data get entity @n[tag=mm_$(name)_spawner,tag=mm_special_element] equipment.feet.components."minecraft:custom_data".Cooldown
function mm:dialog/settings/map/map_features/entry/cooldown with storage mm:main temporary
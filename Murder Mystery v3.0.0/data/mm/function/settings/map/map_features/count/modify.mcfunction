...
$execute store success score success_count mm_main run function mm:settings/map/map_features/count/resolve {name:$(name),count:$(count)}
...
$tellraw @s {"translate":"mm.feedback.map.map_features.count.set.success","color":"green","with":[{"translate":"mm.text_settings.map.map_features.$(name)","color":"aqua"},{"text":"$(count)","color":"gold"}]}
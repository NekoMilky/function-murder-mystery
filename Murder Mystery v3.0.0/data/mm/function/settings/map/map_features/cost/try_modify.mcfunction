...
$execute at @s store result storage mm:main temporary.current_cost int 1 run data get entity @n[tag=mm_$(name)_spawner,tag=mm_special_element] equipment.feet.components."minecraft:custom_data".Cost
function mm:dialog/settings/map/map_features/entry/cost with storage mm:main temporary